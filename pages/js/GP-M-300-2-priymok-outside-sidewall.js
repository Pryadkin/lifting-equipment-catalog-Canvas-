...
// Добавление картинки (схемы подъемника) на холст
  const pic = new Image();
  pic.src = 'img/GP-M-300-2-priymok-outside-sidewall.jpg';
...
let makeGap = function() {
  ...
  ctx.fillRect(x, y + 57, 50, 31); // Стираем фон внизу
  ctx.fillRect(x + 290, y + 57, 50, 31);
  ctx.fillRect(x + 346, y + 20, 35, 50);
  ctx.fillRect(x + 350, y - 330, 35, 100);
  ctx.fillRect(x + 620, y - 440, 50, 31);

  getNum(x, y + 85, gap()); // Рисуем монтажные зазоры
  getNum(x + 290, y + 85, gap());
  getRotate(x + 345, y + 70, gap());
  getRotate(x + 350, y - 240, 180 + gap()); // 180 - глубина мачты
  getNum(x + 620, y - 410, gap());
}
...
let kpu = function() {
  let x1 = 290; // Координаты нижнего КПУ
      y1 = 950;
      x2 = 578; // Координаты верхнего КПУ
      y2 = 420;
      a = 30; // ширина КПУ
      b = 60; // высота КПУ
...
function infrontFunk_1() {
  ...
  canvas_arrow(ctx, x + 40, y + 30, x, y + 30);
    ...
    ctx.fillStyle = 'white'; //закрашиваем фон под размером в белый цвет
    ctx.fillRect(x - 5, y - 15, 50, 60); //очистим от предыдущего размера
  }
}
