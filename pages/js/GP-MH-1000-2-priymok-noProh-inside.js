...
// Добавление картинки (схемы подъемника) на холст
  const pic = new Image();
  pic.src = 'img/GP-MH-1000-2-priymok-noProh-inside.jpg';
...
let makeGap = function() {
  ...
  ctx.fillRect(x - 50, y + 7, 120, 35); //очистим от предыдущего размера (120, 35 - горизонтальное стирание)
  ctx.fillRect(x + 320, y + 7, 120, 35);
  ctx.fillRect(x + 690, y + 7, 120, 35);
  ctx.fillRect(x + 1070, y + 7, 60, 35);

  getNum(x + 20, y + 40, gap()); // Рисуем монтажные зазоры
  getNum(x + 320, y + 40, gap());
  getNum(x + 770, y + 40, gap());
  getNum(x + 1070, y + 40, gap());

  ctx.fillRect(x - 50, y + 627, 120, 35); // Стираем фон внизу
  ctx.fillRect(x + 320, y + 627, 120, 35);
  ctx.fillRect(x + 690, y + 627, 120, 35);
  ctx.fillRect(x + 1070, y + 627, 60, 35);

  getNum(x + 20, y + 660, gap()); // Рисуем нижние монтажные зазоры
  getNum(x + 320, y +660, gap());
  getNum(x + 770, y + 660, gap());
  getNum(x + 1070, y + 660, gap());
}
...
// КПУ
let kpu = function() {
  let x1 = 250; // Координаты нижнего КПУ
      y1 = 1040;
      x2 = 250; // Координаты верхнего КПУ
      y2 = 480;
      a = 30; // ширина КПУ
      b = 60; // высота КПУ
...
let setUpListener = function() { // Обработчик события прорисовки ЩУ
  ...
  let getShit = function(direction) {
    ...
    if(direction === 'up') {
      ...
      clearShit(x, y + 600); // Очистить нижний рисунок ЩУ
      getShit(x, y); // Нарисовать верхний рисунок ЩУ
    }
    if(direction === 'down') {
      clearShit(x, y); // Очистить верхний рисунок ЩУ
      clearShit(x, y + 600); // Очистить нижний рисунок ЩУ
      getShit(x, y + 600); // Нарисовать нижний рисунок ЩУ
    }
