...
// Добавление картинки (схемы подъемника) на холст
  var pic = new Image();
  pic.src = 'img/GP-MH-1000-2-serv-proh-outside.jpg';
...
let makeGap = function() {
  ...
  ctx.fillStyle = 'white';
  // ctx.fillRect(x - 50, y + 7, 120, 35);
  ctx.fillRect(x + 320, y + 7, 120, 35);//очистим от предыдущего размера (120, 40 - горизонтальное стирание)
  // ctx.fillRect(x + 580, y + 7, 120, 35);
  // ctx.fillRect(x + 940, y + 7, 120, 35);
  // getNum(x + 20, y + 40, gap());
  getNum(x + 320, y + 40, gap());
  // getNum(x + 660, y + 40, gap());
  // getNum(x + 960, y + 40, gap());
}
...
// Высота приямка
  ...
  function prFunk() {
    ...
    ctx.fillStyle = 'white';
    ctx.fillRect(x, y, 40, 120); //очистим от предыдущего размера (40, 120 - вертикальное стирание)
    getRotate(x + 10, y + 90, prText.value);
  }
...
let prWidth = function() {
  ...
  getNum(x + 30, y + 30, +cabWidthText.value + a*2 + b*2); // +cabWidthText.value - значение ширины кабины переведенное в число знаком(+)
}
...
// Глубина кабины
  ...
  let prDepth = function() {
    ...
    ctx.fillStyle = 'white';
    ctx.fillRect(x, y, 120, 40); //очистим от предыдущего размера (120, 40 - горизонтальное стирание)
    getNum(x + 30, y + 30, +cabDepthText.value + a*2 + b*2); // +cabDepthText.value - значение ширины кабины переведенное в число знаком(+)
  }
...
// Высота кабины
  ...
  function cabHeightFunk() {
    ...
    getRotate(x + 5, y - 20, cabHeightText.value);
...
let gapCeiling = function() {
  ...
  getRotate(x, y, ''); // Зазор до потолка - ''мм
}
gapCeiling();
...
let kpu = function() {
  let x1 = 240; // Координаты нижнего КПУ
      y1 = 1020;
      x2 = 550; // Координаты верхнего КПУ
      y2 = 480;
      a = 30; // ширина КПУ
      b = 60; // высота КПУ
