...
// Добавление картинки (схемы подъемника) на холст
  const pic = new Image();
  pic.src = 'img/GP-M-C-1000-2-priymok-outside-sidewall.jpg';
...
let makeGap = function() {
  ...
  ctx.fillRect(x + 190, y - 385, 50, 31); // Стираем фон внизу
  // ctx.fillRect(x + 290, y, 50, 31);
  // ctx.fillRect(x, y - 90, 35, 50);
  // ctx.fillRect(x + 405, y - 440, 35, 100);
  // ctx.fillRect(x + 580, y - 410, 50, 31);

  getNum(x + 190, y - 355, gap()); // Рисуем монтажные зазоры
  // getNum(x + 290, y + 31, gap());
  // getRotate(x, y - 40, gap());
  // getRotate(x + 405, y - 340, 320 + gap()); // 180 - глубина мачты
...
// КПУ
let kpu = function() {
  let x1 = 365; // Координаты нижнего КПУ
      y1 = 850;
      x2 = 670; // Координаты верхнего КПУ
      y2 = 420;
      a = 30; // ширина КПУ
      b = 60; // высота КПУ
...
let setUpListener = function() { // Обработчик события прорисовки ЩУ
  ...
  let getShit = function(direction) {
    ...
    if(direction === 'up') {
      ...
      clearShit(x, y + 400); // Очистить нижний рисунок ЩУ
      getShit(x, y); // Нарисовать верхний рисунок ЩУ
    }
    if(direction === 'down') {
      clearShit(x, y); // Очистить верхний рисунок ЩУ
      clearShit(x, y + 400); // Очистить нижний рисунок ЩУ
      getShit(x, y + 400); // Нарисовать нижний рисунок ЩУ
    }
