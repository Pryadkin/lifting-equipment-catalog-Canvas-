...
// Добавление картинки (схемы подъемника) на холст
  const pic = new Image();
  pic.src = 'img/GP-M-1500-2-serv-inside-sidewall.jpg';
...
let makeGap = function() {
  ...
  ctx.fillRect(x + 90, y - 385, 50, 31); // Стираем фон внизу
  // ctx.fillRect(x + 290, y, 50, 31);
  ctx.fillRect(x, y - 150, 35, 50);
  ctx.fillRect(x + 590, y - 380, 35, 100);
  // ctx.fillRect(x + 580, y - 410, 50, 31);

  getNum(x + 90, y - 355, gap()); // Рисуем монтажные зазоры
  // getNum(x + 290, y + 31, gap());
  getRotate(x, y - 100, gap());
  getRotate(x + 590, y - 280, 320 + gap()); // 180 - глубина мачты
...
// Ширина платформы
  ...
  function cabWidthFunk() {
    ...
    getNum(x + 10, y + 30, cabWidthText.value);
    prWidth();
  }
  ...
  let prDepth = function() {
    ...
    count++;
    ctx.fillStyle = 'white';
    ctx.fillRect(x + 20, y, 120, 35); //очистим от предыдущего размера (120, 40 - горизонтальное стирание)
...
// КПУ
let kpu = function() {
  let x1 = 375; // Координаты нижнего КПУ
      y1 = 800;
      x2 = 375; // Координаты верхнего КПУ
      y2 = 450;
      a = 30; // ширина КПУ
      b = 60; // высота КПУ
...
let setUpListener = function() { // Обработчик события прорисовки ЩУ
  ...
  let getShit = function(direction) {
    ...
    if(direction === 'up') {
      ...
      clearShit(x, y + 300); // Очистить нижний рисунок ЩУ
      getShit(x, y); // Нарисовать верхний рисунок ЩУ
    }
    if(direction === 'down') {
      clearShit(x, y); // Очистить верхний рисунок ЩУ
      clearShit(x, y + 300); // Очистить нижний рисунок ЩУ
      getShit(x, y + 300); // Нарисовать нижний рисунок ЩУ
    }
...
// Функция пишет число из строки ввода
  ...
  function fenceBehindFunk() {
    ...
    // на виде сверху
    ctx.lineWidth = 6;
    ctx.moveTo(0, 375 + 65 + 50);
    ctx.lineTo(186, 375 + 65 + 50);
    ctx.stroke();
  ...
  function fenceRightFunk() {
    ...
    //на виде сверху
    ctx.lineWidth = 6;
    ctx.moveTo(185, 375 + 63 + 50);
    ctx.lineTo(185, 587 + 50);
    ctx.stroke();
...
function rightFunk_1() {
  ...
  if(arrowRight_1.classList.contains('active')) {
    ...
    ctx.fillRect(x + 292, y - 15, 55, 60); //очистим от предыдущего размера
  }
}
...
function leftFunk_1() {
  ...
  canvas_arrow(ctx, x + 40, y + 30, x, y + 30);
    ...
    ctx.fillStyle = 'white'; //закрашиваем фон под размером в белый цвет
    ctx.fillRect(x - 8, y - 15, 55, 60); //очистим от предыдущего размера
  }
}
