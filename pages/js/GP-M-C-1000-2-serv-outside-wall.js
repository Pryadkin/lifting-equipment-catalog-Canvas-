...
// Добавление картинки (схемы подъемника) на холст
  const pic = new Image();
  pic.src = 'img/GP-M-C-1000-2-serv-outside-wall.jpg';
...
function rightFunk_1() {
  ...
  if(arrowRight_1.classList.contains('active')) {
    ...
    ctx.fillRect(x + 293, y - 15, 53, 60); //очистим от предыдущего размера
  }
}
...
function leftFunk_1() {
  ...
  canvas_arrow(ctx, x + 40, y + 30, x, y + 30);
    ...
    ctx.fillStyle = 'white'; //закрашиваем фон под размером в белый цвет
    ctx.fillRect(x - 8, y - 15, 53, 60); //очистим от предыдущего размера
  }
}
